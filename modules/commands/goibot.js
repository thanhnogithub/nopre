...
module.exports.handleEvent = function({ api, event, args, Threads }) {
  ...
  var tl = ["chào bạn tôi là bot của Thành", "bạn kêu tôi có việc gì?", "tôi yêu bạn vai lon", "Yêu em <3", "Hi, chào con vợ bé:3", "Vợ gọi có việc gì không?", "Sử dụng callad để liên lạc với admin!", "Em là bot cute nhất hành tinh", "Nói gì thế con lợn", "Em đây~~~~"];
  var rand = tl[Math.floor(Math.random() * tl.length)]

 if ((event.body.toLowerCase() == "bot out") || (event.body.toLowerCase() == "Bot out")) {
    return api.sendMessage("byeeeeeee", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
   if ((event.body.toLowerCase() == "bot ngu") || (event.body.toLowerCase() == "Bot ngu")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua fb : https://www.facebook.com/iamptthanh", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
     if ((event.body.toLowerCase() == "bot ncc") || (event.body.toLowerCase() == "Bot ncc")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua fb : https://www.facebook.com/iamptthanh", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
  if ((event.body.toLowerCase() == "bot cút mẹ đi") || (event.body.toLowerCase() == "Bot cút mẹ đi")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua fb : https://www.facebook.com/iamptthanh", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
  if ((event.body.toLowerCase() == "bot cút") || (event.body.toLowerCase() == "Bot cút")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua fb : https://www.facebook.com/iamptthanh", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
   ...
   if ((event.body.toLowerCase() == "đm bot") || (event.body.toLowerCase() == "Đm bot")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua fb : https://www.facebook.com/iamptthanh", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
...
if ((event.body.toLowerCase() == "dmm") ||  (event.body.toLowerCase() == "Dmm")) {
    return api.sendMessage("Lớn rồi văn minh lên bạn đừng để bị ăn chửi =)", threadID);
  };
if ((event.body.toLowerCase() == "đmm") ||  (event.body.toLowerCase() == "Đmm")) {
    return api.sendMessage("Lớn rồi văn minh lên bạn đừng để ăn chửi", threadID);
  };
